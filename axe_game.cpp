...
int main()
{
    ...
    /*
            (bottom_axe_edge >= upper_circle_edge) && 
            (upper_axe_edge <= bottom_circle_edge) && 
    ...
    */
    ...
    while (WindowShouldClose() != true) // or while(WindowShouldClose() == false)
    {
        ...
        {
            DrawText("Game Over!", 520, 200, 42, RED);
        }
        else
        {
            // Game Logic Begins

            //Update The Circle Edges Every Frame
            left_circle_edge = circle_x - circle_rad;
            right_circle_edge = circle_x + circle_rad;
            upper_circle_edge = circle_y - circle_rad;
            bottom_circle_edge = circle_y + circle_rad;
            //Update the Axe Edges Every Frame
            left_axe_edge = axe_x;
            right_axe_edge = axe_x + axe_width;
            upper_axe_edge = axe_y;
            bottom_axe_edge = axe_y + axe_height;

            //Update Collision with Axe Every Frame - Check
            collision_with_axe = 
                (bottom_axe_edge >= upper_circle_edge) && 
                (upper_axe_edge <= bottom_circle_edge) && 
                (left_axe_edge <= right_circle_edge) && 
...
/*
    RayLib Functions:
        ...
        DrawCircle();
        DrawText(text, posX, posY, fontSize, color); - Brings a Text Box to the Screen
